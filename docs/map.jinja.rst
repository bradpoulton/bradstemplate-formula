...
We will see a quick setup to configure the ``bradstemplate`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``bradstemplate`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``bradstemplate`` formulas under ``/srv/salt/bradstemplate/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/bradstemplate/parameters/dns:domain/
    ln -s dns:domain /srv/salt/bradstemplate/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/bradstemplate/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/bradstemplate/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``bradstemplate`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``bradstemplate`` formulas under ``/srv/salt/bradstemplate/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/bradstemplate/parameters/roles

We will define 2 roles:

- ``bradstemplate/server``
- ``bradstemplate/client``

We create a configuration for the role ``bradstemplate/server`` in ``/srv/salt/bradstemplate/parameters/roles/bradstemplate/server.yaml``:
...
We create another configuration for the role ``bradstemplate/client`` in ``/srv/salt/bradstemplate/parameters/roles/bradstemplate/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles bradstemplate/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles bradstemplate/client
...
.. code-block:: console
    ...
    mv /srv/salt/bradstemplate/parameters/dns:domain/  '/srv/salt/bradstemplate/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-bradstemplate-as-json:
      test.nop:
        - name: {{ bradstemplate | json }}
...
Here is an example based on `template-formula/bradstemplate/config/file.sls`_:
...
This ``sls`` file expose a ``bradstemplate`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ bradstemplate['winner'] }}
...
.. _template-formula/bradstemplate/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/bradstemplate/config/file.sls
